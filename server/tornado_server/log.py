...
def tornado_multi_process_setup(log_directory: str):
    format = '[%(asctime)s] %(levelname)s \t [%(filename)s %(lineno)d] %(message)s'
    ...
    server_logger.addHandler(stream_handler)
    server_info_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'server.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
    ...
    apscheduler_logger.addHandler(stream_handler)
    apscheduler_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'apscheduler.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
    ...
    es_logger = logging.getLogger("elasticsearch")
    es_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'elasticsearch.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
    ...
    debug_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'debug.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
    ...
    info_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'info.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
    ...
    error_file_handler = TimedRotatingFileHandler(
        filename=os.path.join(log_directory, 'error.log.{}'.format(os.getpid())),
        encoding='utf-8',
        when='D',
        interval=1,
        backupCount=7
